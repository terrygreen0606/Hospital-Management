...
export default {
    ...
    methods: {
        getOrders(page = 1) {
            ...
            this.axios.get("/api/getPharmacyPatientIDs/"+this.$auth.user().id).then(res => {

                let p = page == 1 ? "" : "&page=" + page;
                let queries = { page: page, ids: res.data };
                let stringifiedQuery = QueryString.stringify(queries);
                this.axios
                    .get(`/api/orders?ids=` + JSON.stringify(res.data) + p)
                    .then(response => {
                        ...
                        for (let counter = 0; counter < response.data.data.length; counter++) {
                            ...
                            if(trackingNumber == 'N/A') continue;
